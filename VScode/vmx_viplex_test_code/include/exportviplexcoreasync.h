...
extern "C"
{
    ...
    */
	...
	VIPLEX_EXPORT void nvPing(ExportViplexCallback  callback);
...
#pragma region UpgradeManager
	...
	* @number 99
	*/
	void nvQueryUpdateFileByTypeAsync(const char *data, ExportViplexCallback  callback);
	/**
	 * showdoc
	 * @catalog 接口示例/升级管理
	 * @title 搜索线上升级包
	 ...
	 * @number 99
	 */
	void nvGetOnlineUpgradeFileAsync(const char *data, ExportViplexCallback  callback);

	void nvDownloadUpgradeFileAsync(const char *data, ExportViplexCallback  callback);
	/**
	* showdoc
	* @catalog 接口示例/升级管理
	* @title 停止下载
	...
	* @number 99
	*/
	void nvStopDownloadUpgradeFileAsync(const char *data, ExportViplexCallback  callback);
	/**
	 * showdoc
	 * @catalog 接口示例/升级管理
	 * @title 获取升级描述文件内容
	 ...
	 * @number 99
	 */
	void nvReadFileContentFromZipFileAsync(const char *data, ExportViplexCallback  callback);

	void nvUpdateAppAsync(const char *data, ExportViplexCallback  callback);

	void nvUpdateOSAsync(const char *data, ExportViplexCallback  callback);
	...
	* @number 99
	*/
	void nvStopCurrentUpdateTaskAsync(const char *data, ExportViplexCallback  callback);
	/**
	* showdoc
	* @catalog 接口示例/升级管理
	* @title 包是否可升级判断接口
...
* @description 简单天气预报，对应媒体类型：SIMPLE_WEATHER。支持只显示文字信息的天气预报，文字属性只支持统一修改。
* @url ""
* @json_param {"programID":1,"pageID":1,"pageInfo":{"id":1,"name":"节目01","widgetContainers":[{"contents":{"widgets":[{"constraints":[{"cron":["0 0 0 ? * 1,2,3,4,5,6,7"],"endTime":"4017-12-30T23:59:59Z+8:00","startTime":"1970-01-01T00:00:00Z+8:00"}],"duration":5000,"metadata":{"area":{"city":"西安","country":"中国","county":"西安","province":"陕西省"},"effects":{"animation":"MARQUEE_LEFT"},"isShowInOneLine":false,"refreshPeriod":300000,"regular":"天气:$w\n温度:$t\n风力:$b级\n当前温度:$c\n湿度:$h","temperatureUnit":"Celsius","textAttributes":{"font":{"family":["Times","Georia","New York"]},"textColor":"#ffff0000"}},"type":"SIMPLE_WEATHER"}]}}]}}
...
*/
    ...
    * @json_param {"programID":1,"outPutPath":"test","mediasPath":[{"oldPath":"test","newPath":"test"}]}
 ...
 * @param lang 必选 string 语言,英文:en,中文:zh-cn,日文:jp,韩文:kr,西班牙语:es,法语:fr
 * @return {"serverInfo":[{"islocal":true,"label":"aa","node":"www.baidu.com"}]}
 * @return_param islocal bool 外部存储空间总大小,单位G
 * @return_param diskAvailableSize float 是否是用户自定义添加的数据,true:用户自定义,false:服务器获取
 * @return_param label string 节点名称
 * @return_param node string 节点地址
 * @return_param code int 错误码:`0`获取成功`65535`请求超时
 * @remark
...
#pragma endregion 其他  待补充
    ...
    */
    ...
    VIPLEX_EXPORT void nvDownloadTerminalAllLog(const char *data, ExportViplexCallback callback);
        /**
    * showdoc
    * @catalog 接口示例/终端日志
    * @title 获取终端播放日志
    ...
    * @number 99
    */
    VIPLEX_EXPORT void nvDownloadTerminalPlayLog(const char *data,ExportViplexCallback callback);
